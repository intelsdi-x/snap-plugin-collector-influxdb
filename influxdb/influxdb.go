...
func (ic *InfluxdbCollector) getDiagnostics() ([]plugin.Metric, error) {
	...
	response, err := getResponse(ic.urlDiagnostic.String())
	if err != nil {
...
func (ic *InfluxdbCollector) getStatistics() ([]plugin.Metric, error) {
	mts := []plugin.Metric{}
	var stats stats
	response, err := getResponse(ic.urlStatistic.String())
	if err != nil {
		log.Errorf("error getting response err=%v response=%v", err.Error(),
			response)
		return nil, err
...
func (m *InfluxdbCollector) InitURLs(host string, port int64, user string, passwd string) error {
	errs := []error{}
	var err error
